(function(global){
  ...
  function mergePack(base, extra){
    ...
    for(i=0;i<sections.length;i++){
      ...
      srcExtra = extra[name] || {};
      out[name] = {};
      for(key in srcBase){ if(own(srcBase, key)) out[name][key] = srcBase[key]; }
      for(key in srcExtra){ if(own(srcExtra, key)) out[name][key] = srcExtra[key]; }
    }
    return out;
  }
  ...
  function ensureLang(lang){
    ...
    cache[code] = mergePack(base, loadRawLang(code) || emptyPack());
    return cache[code];
  }
  ...
  function get(path, data, lang){
    ...
    if(typeof value === 'undefined') value = getByPath(ensureBase(), path);
  ...
  function setLang(lang){
    ...
    try{ global.document && global.document.documentElement && global.document.documentElement.setAttribute('lang', currentLang); }catch(_){ }
    try{
      var evt;
      if(typeof CustomEvent === 'function') evt = new CustomEvent('i18n:changed', { detail:{ lang: currentLang } });
